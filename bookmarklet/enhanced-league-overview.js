...
javascript: (function(){
    ...
    var hdl = {
        ...
        attachAdditionalInfo: function() {
            if(window._hdl_curtDate) {
                hdl.curtDate = window._hdl_curtDate;
                hdl.curtDateStr = window._hdl_curtDateStr;
            } else {
                var sdbateMatch = jQuery('#scoreboard .scoreboard .current .Navtarget').attr('href').match(/sbdate=(\d{4})-(\d{2})-(\d{2})/i);
                var yr = parseInt(sdbateMatch[1], 10), month = parseInt(sdbateMatch[2], 10) -1, day = parseInt(sdbateMatch[3], 10);
                hdl.curtDate = new Date(yr, month, day, 0, 0, 0, 0);
                ...
                window._hdl_curtDate = hdl.curtDate;
                window._hdl_curtDateStr = hdl.curtDateStr;
            }
